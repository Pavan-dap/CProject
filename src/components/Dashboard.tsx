...
import { Row, Col, Card, Statistic, Progress, Table, Tag, Avatar, Typography, Space, List } from 'antd';
import { 
  ProjectOutlined, 
  CheckSquareOutlined, 
  UserOutlined, 
...
const Dashboard: React.FC = () => {
  ...
  const overdueTasks = userTasks.filter(t => new Date(t.dueDate) < new Date() && t.status !== 'completed').length;
  ...
  const recentActivities = [
    { action: 'Window installation completed', project: 'ABC Township', time: '2 hours ago', type: 'success' },
    { action: 'New task assigned', project: 'Green Valley', time: '4 hours ago', type: 'info' },
    { action: 'Quality check failed', project: 'ABC Township', time: '6 hours ago', type: 'error' },
    { action: 'Material delivery received', project: 'Green Valley', time: '1 day ago', type: 'success' }
  ];
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card>
            <Statistic
              ...
              suffix={completedProjects > 0 ? `(${completedProjects} completed)` : ''}
            />
          </Card>
        </Col>
        <Col xs={24} sm={12} lg={6}>
                    ...
                    <List.Item>
                      <div>
                        <div style={{ fontSize: '13px' }}>
                          <Text type={item.type === 'error' ? 'danger' : 'secondary'}>
                            {item.action}
                          </Text>
                        </div>
