...
const ProjectStatusReport: React.FC<ProjectStatusReportProps> = ({ 
  ...
  const blockColumns = [
    ...
    },
    {
      ...
      key: 'totalUnits',
      render: (record: any) => {
        const totalUnits = record.floors.reduce((sum: number, floor: any) => 
          sum + floor.units.length, 0
        );
        return totalUnits;
      }
    },
  ...
  return (
    <div style={{ padding: isPublic ? 0 : 24 }}>
      {!isPublic && (
        <div style={{ 
          display: 'flex', 
          ...
          <Row gutter={[24, 24]} align="middle">
            <Col xs={24} md={16}>
              ...
              <div style={{ marginTop: 8 }}>
                ...
                <Divider type="vertical" />
                <Text type="secondary">
                  {dayjs(project.startDate).format('MMM DD, YYYY')} - {dayjs(project.endDate).format('MMM DD, YYYY')}
                </Text>
              </div>
              <div style={{ marginTop: 8 }}>
                <Tag color="blue">{project.buildings} Blocks</Tag>
                <Tag color="green">{project.floors} Floors per Block</Tag>
                <Tag color="orange">{project.units.toLocaleString()} Total Units</Tag>
              </div>
            </Col>
            <Col xs={24} md={8}>
                  ...
                  {block.floors.map(floor => (
                    <Col xs={24} sm={12} md={8} key={floor.floorNumber}>
                      <Card size="small" title={`Floor ${floor.floorNumber}`}>
                        ...
                        <Text type="secondary">
                          {floor.units.length} units • {floor.units.filter(u => u.completionPercentage === 100).length} completed
                        </Text>
                        
                        <div style={{ marginTop: 12 }}>
                          <Text strong style={{ fontSize: '12px' }}>Units:</Text>
                            ...
                            {floor.units.map(unit => (
                              <div
                                ...
                                style={{
                                  ...
                                  height: 24,
                                  borderRadius: 4,
                                  backgroundColor: unit.completionPercentage === 100 ? '#52c41a' : 
                                                 unit.completionPercentage > 0 ? '#1890ff' : '#d9d9d9',
                                  display: 'flex',
                                  alignItems: 'center',
                  ...
                  <Col xs={24} md={16}>
                    <div style={{ marginBottom: 8 }}>
                      ...
                      <div style={{ marginTop: 4 }}>
                        ...
                        {task.unitType && <Tag color="orange">{task.unitType}</Tag>}
                        {getStatusIcon(task.status)}
                        <Text style={{ marginLeft: 8 }}>{task.status.replace('-', ' ').toUpperCase()}</Text>
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                      {(task.photos || []).slice(0, 3).map((photo, index) => (
                        <div key={index} style={{ 
                          ...
                          <CameraOutlined style={{ fontSize: 16, color: '#999' }} />
                        </div>
                      ))}
