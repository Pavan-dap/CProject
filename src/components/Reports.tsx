...
const Reports: React.FC = () => {
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      const projectName = selectedProject === 'all' ? 'All_Projects' :
        projects.find(p => p.id === selectedProject)?.name?.replace(/\s+/g, '_') || 'Unknown_Project';
      const fileName = `${projectName}_Report_${dayjs().format('YYYY-MM-DD')}.pdf`;
  ...
  const filteredTasks = tasks.filter(task => {
    ...
    const isUserTask = user?.role === 'admin' || 
  ...
  const taskStatusData = [
    { name: 'Not Started', value: filteredTasks.filter(t => t.status === 'not-started').length, color: '#d9d9d9' },
    { name: 'In Progress', value: inProgressTasks, color: '#1890ff' },
    { name: 'Completed', value: completedTasks, color: '#52c41a' },
    { name: 'On Hold', value: filteredTasks.filter(t => t.status === 'on-hold').length, color: '#faad14' }
  ];

  const priorityData = [
    { name: 'High', value: filteredTasks.filter(t => t.priority === 'high').length, color: '#f5222d' },
    { name: 'Medium', value: filteredTasks.filter(t => t.priority === 'medium').length, color: '#faad14' },
    { name: 'Low', value: filteredTasks.filter(t => t.priority === 'low').length, color: '#52c41a' }
  ];
  ...
  const getProgressTimelineData = () => {
    const last7Days = Array.from({ length: 7 }, (_, i) => {
      ...
      return {
        ...
        cumulative: filteredTasks.filter(task => 
          task.status === 'completed' && 
  ...
  const handleExport = (format: 'excel' | 'pdf') => {
    ...
    const data = {
      reportType,
      dateRange: dateRange ? `${dateRange[0].format('YYYY-MM-DD')} to ${dateRange[1].format('YYYY-MM-DD')}` : 'All time',
    ...
    link.href = url;
    link.download = `construction-report-${dayjs().format('YYYY-MM-DD')}.${format === 'excel' ? 'json' : 'json'}`;
    link.click();
        ...
        <Col xs={24} sm={6}>
          <Card>
            <Statistic
              ...
              suffix="%"
              prefix={<CheckSquareOutlined />}
              valueStyle={{ color: completionRate > 70 ? '#52c41a' : completionRate > 40 ? '#faad14' : '#f5222d' }}
            />
          </Card>
        </Col>
        <Col xs={24} sm={6}>
          ...
          <Card title="Task Status Distribution">
            ...
            <div style={{ textAlign: 'center', marginTop: 16 }}>
              {taskStatusData.map((entry) => (
                <Tag key={entry.name} color={entry.color} style={{ margin: '2px' }}>
          ...
          <Card title="Progress Timeline (Last 7 Days)">
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={timelineData}>
                ...
                <YAxis />
                <Tooltip />
                <Line type="monotone" dataKey="completed" stroke="#52c41a" name="Daily Completed" />
                <Line type="monotone" dataKey="cumulative" stroke="#1890ff" name="Cumulative" />
              </LineChart>
            </ResponsiveContainer>
          </Card>
        </Col>
