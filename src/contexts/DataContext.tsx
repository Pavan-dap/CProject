...
const initialProjects: Project[] = [
  {
    ...
    status: "in-progress",
    description:
      "Supply and installation of UPVC windows and doors for 1000 residential units",
  },
  {
    id: 2,
...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const getProjectHierarchy = (projectId: number): ProjectHierarchy => {
    ...
    projectTasks.forEach((task) => {
      ...
      const floorNumber = task.floor
        ? parseInt(task.floor.replace(/\D/g, "")) || 0
        : 0;
    ...
    const blocks = Array.from(blocksMap.entries()).map(
      ([blockName, floorsMap]) => {
        const floors = Array.from(floorsMap.entries()).map(
          ([floorNumber, unitsMap]) => {
            const units = Array.from(unitsMap.values()).map((unit) => {
              const totalProgress = unit.tasks.reduce(
                (sum, task) => sum + task.progress,
                0
              );
              const completionPercentage =
                unit.tasks.length > 0 ? totalProgress / unit.tasks.length : 0;
              return { ...unit, completionPercentage };
            });

            const floorCompletion =
              units.length > 0
                ? units.reduce(
                    (sum, unit) => sum + unit.completionPercentage,
                    0
                  ) / units.length
                : 0;
        ...
        const blockCompletion =
          floors.length > 0
            ? floors.reduce(
                (sum, floor) => sum + floor.completionPercentage,
                0
              ) / floors.length
            : 0;
    ...
    const overallCompletion =
      blocks.length > 0
        ? blocks.reduce((sum, block) => sum + block.completionPercentage, 0) /
          blocks.length
        : 0;
  ...
  const addTaskComment = useCallback(
    (taskId: number, comment: Omit<Comment, "id">) => {
  ...
  const updateUser = useCallback(
    (id: number, updates: Partial<User>) => {
      setUsers((prev) => {
        ...
        if (updatedUser) {
          globalEventBus.emit("app:userUpdated", {
            user: updatedUser,
            timestamp: Date.now(),
          });
        }
  ...
  const deleteUser = useCallback(
    (id: number) => {
      setUsers((prev) => {
        ...
        globalEventBus.emit("app:userDeleted", {
          userId: id,
          timestamp: Date.now(),
        });
        syncManager.notifyAll();
