...
(1, 'Foundation Work - Block A', 'Complete foundation work for Block A including excavation and concrete pouring', 1, 1, 4, 3, 'completed', 'high', 100.00, 240.00, 235.00, '2024-01-20', '2024-01-30', '2024-01-28', 1, NOW(), NOW()),
(2, 'Structural Framework - Block A Floor 1-5', 'Complete structural framework for floors 1-5 in Block A', 1, 2, 4, 3, 'in-progress', 'high', 75.00, 480.00, 360.00, '2024-01-25', '2024-03-15', NULL, 0, NOW(), NOW()),
(3, 'Foundation Work - Block B', 'Complete foundation work for Block B', 1, 11, 5, 3, 'in-progress', 'high', 60.00, 240.00, 144.00, '2024-02-01', '2024-02-15', NULL, 1, NOW(), NOW()),

-- Utility Installation Tasks (Depend on Structural)
(4, 'Plumbing Rough-in - Block A Floor 1', 'Install plumbing rough-in for all units on Floor 1', 1, 1, 4, 3, 'not-started', 'medium', 10.00, 120.00, 15.00, '2024-02-10', '2024-02-20', NULL, 1, NOW(), NOW()),
(5, 'Electrical Rough-in - Block A Floor 1', 'Complete electrical rough-in installation for Floor 1', 1, 1, 4, 3, 'not-started', 'high', 0.00, 100.00, 0.00, '2024-02-15', '2024-03-10', NULL, 1, NOW(), NOW()),

-- Window and Door Installation Tasks (Depend on Utilities)
(6, 'UPVC Window Installation - Block A Floor 1 3BHK Units', 'Install UPVC windows in 3BHK units (101, 102)', 1, 1, 4, 3, 'in-progress', 'high', 40.00, 80.00, 32.00, '2024-02-01', '2024-02-25', NULL, 0, NOW(), NOW()),
(7, 'Door Installation - Block A Floor 1 2BHK Units', 'Install doors in 2BHK units (103, 104)', 1, 3, 4, 3, 'not-started', 'medium', 0.00, 60.00, 0.00, '2024-02-20', '2024-03-05', NULL, 0, NOW(), NOW()),
(8, 'Window Installation - Block A Floor 1 1BHK Units', 'Install windows in 1BHK unit (105)', 1, 5, 4, 3, 'completed', 'medium', 100.00, 40.00, 38.00, '2024-02-05', '2024-02-15', '2024-02-14', 0, NOW(), NOW()),

-- Finishing Tasks (Depend on Windows/Doors)
(9, 'Interior Finishing - Block A Floor 1', 'Complete interior finishing work for all Floor 1 units', 1, 1, 4, 3, 'not-started', 'medium', 0.00, 200.00, 0.00, '2024-03-01', '2024-03-20', NULL, 0, NOW(), NOW()),
(10, 'Quality Inspection - Block A Floor 1', 'Final quality inspection for all completed units', 1, 1, 4, 3, 'not-started', 'high', 0.00, 40.00, 0.00, '2024-03-15', '2024-03-25', NULL, 0, NOW(), NOW()),

-- Green Valley Complex Tasks
(11, 'Site Preparation - Green Valley Block A', 'Prepare construction site for Green Valley Block A', 2, 16, 5, 3, 'completed', 'high', 100.00, 80.00, 75.00, '2024-03-01', '2024-03-10', '2024-03-08', 1, NOW(), NOW()),
(12, 'Foundation - Green Valley Block A', 'Foundation work for Green Valley Block A', 2, 16, 5, 3, 'in-progress', 'high', 60.00, 200.00, 120.00, '2024-03-05', '2024-03-25', NULL, 0, NOW(), NOW());
...
-- SELECT p.name, COUNT(t.id) as task_count, AVG(t.progress) as avg_progress 
-- FROM projects_project p 
-- LEFT JOIN tasks_task t ON p.id = t.project_id 
-- GROUP BY p.id, p.name;

-- SELECT t.title, t.status, t.progress, GROUP_CONCAT(dt.title) as dependencies
-- FROM tasks_task t
-- LEFT JOIN tasks_taskdependency td ON t.id = td.task_id
-- LEFT JOIN tasks_task dt ON td.depends_on_id = dt.id
-- GROUP BY t.id, t.title, t.status, t.progress;